...
public class BlockKCelOniGar extends BlockFlower {
   ...
   public BlockKCelOniGar(int par1) {
      super(par1);
   ...
   public void registerIcons(IconRegister par1IconRegister) {
      ...
      for(int i = 0; i < this.field_94349_a.length; ++i) {
         if(i < 4) {
            this.field_94349_a[i] = par1IconRegister.registerIcon("forgottennature:KnobCelery_" + i);
         } else if(i > 3 && i < 8) {
            this.field_94349_a[i] = par1IconRegister.registerIcon("forgottennature:Onion_" + (i - 4));
         } else if(i > 7 && i < 12) {
            this.field_94349_a[i] = par1IconRegister.registerIcon("forgottennature:Garlic_" + (i - 8));
         }
      }
   ...
   public void dropBlockAsItemWithChance(World world, int i, int j, int k, int met, float par6, int par7) {
      ...
      met &= 15;
      if(met == 3) {
         this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.foodID], 1, 0));
      } else if(met == 7) {
         this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.foodID], 1, 1));
      } else if(met == 11) {
         this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.foodID], 1, 2));
      }

      for(int n = 0; n < 5; ++n) {
         if(met == 3 && world.rand.nextInt(3) <= met) {
            this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.seedID], 1, 0));
