...
public class BlockGrapeTomCabPine extends BlockFlower {
   ...
   protected boolean canThisPlantGrowOnThisBlockID(int par1) {
      return par1 == Block.tilledField.blockID || par1 == super.blockID;
   }
   ...
   public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int par2, float par3, float par4, float par5) {
      ...
      if(equipped != null && equipped.getItem() == Item.dyePowder && equipped.getItemDamage() == 15) {
         boolean full = false;
         byte var13;
         if(met < 3) {
            var13 = 3;
         ...
         this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.foodID], 1, 14));
      }
   ...
   public void updateTick(World par1World, int par2, int par3, int par4, Random par5Random) {
      ...
      if(par1World.getBlockLightValue(par2, par3 + 1, par4) >= 9) {
         ...
         if(var6 < 14 && par1World.getBlockId(par2, par3 - 1, par4) == super.blockID) {
         ...
         if(par5Random.nextInt(10) == 0 && par1World.getBlockId(par2, par3 - 1, par4) == Block.dirt.blockID && par1World.getBlockId(par2, par3 + 1, par4) == 0 && (var6 == 12 || var6 == 13)) {
            par1World.setBlock(par2, par3 + 1, par4, super.blockID, 14, 2);
         }
   ...
   public void dropBlockAsItemWithChance(World world, int i, int j, int k, int met, float par6, int par7) {
      ...
      met &= 15;
      if(met == 3) {
         this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.foodID], 1, 11));
      } else if(met == 7) {
         this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.foodID], 1, 12));
      } else if(met == 11) {
         this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.foodID], 1, 13));
      ...
      for(int n = 0; n < 5; ++n) {
         if(met == 3 && world.rand.nextInt(3) == 0) {
            this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.seedID], 1, 4));
         } else if(met == 7 && world.rand.nextInt(3) == 0) {
            this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.seedID], 1, 5));
         } else if(met == 11 && world.rand.nextInt(3) == 0) {
            this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.seedID], 1, 6));
         } else if((met == 15 || met == 13) && world.rand.nextInt(3) == 0) {
            this.dropBlockAsItem_do(world, i, j, k, new ItemStack(Item.itemsList[this.seedID], 1, 7));
         }
      }
